...
- [Host backups are filesystem images](#host-backups-are-filesystem-images)
- [What gets inside a host backup](#what-gets-inside-a-host-backup)
- [Why doing this backup](#why-doing-this-backup)
- [How it affects the host platform](#how-it-affects-the-host-platform)
- [When to do the backup](#when-to-do-the-backup)
- [How to backup with Clonezilla](#how-to-backup-with-clonezilla)
  - [Steps for cloning storage drives with Clonezilla](#steps-for-cloning-storage-drives-with-clonezilla)
- [How to restore with Clonezilla](#how-to-restore-with-clonezilla)
...
1. Prepare an USB drive where you can install the Clonezilla Live system. If you use a tool like [Ventoy](https://www.ventoy.net/en/doc_start.html), this is as easy as just copying the Clonezilla ISO file in the USB drive.
...
4. If you have it connected, unplug the USB storage drive used in this guide for storing the backups for the Proxmox virtual machines and templates.

5. Plug in the external USB storage drive where you keep the Clonezilla images.

6. Plug in your Clonezilla Live USB and make your Proxmox VE system boot up with it.

7. Following the [instructions explained in this official Clonezilla guide about **saving a disk image**](https://clonezilla.org/fine-print-live-doc.php?path=clonezilla-live/doc/01_Save_disk_image), create a clone image of each of the internal storage drives in your system.

8. When you have done all the clone images you need, shutdown the system and unplug both the Clonezilla Live USB and the external storage drive holding your Clonezilla backups.

9. As a final verification step, check on a different computer that the images are present in the external storage drive you have used. You should have one folder per each clone image done.
...
2. If you have it connected, unplug the USB storage drive used in this guide for storing the backups for the Proxmox virtual machines and templates.
...
7. Finally, plug back in the other external USB storage drive with the Proxmox VM backups and then boot up normally your system to see if it has been restored properly.

## Final considerations

Be aware that Clonezilla can also make images of partitions. This allows you to consider a backup strategy different from the one shown here. Furthermore, in a restoration of the Proxmox VE system used in this guide, you do not really need to restore the contents of both of the internal storage drives, just the one that truly contains the Proxmox root filesystem. But this also has its danger if the restored system expects a different (_older_) configuration on the secondary drive than the one currently present. Therefore, you must keep the clone images of both internal drives current and _in sync_, so to speak, to avoid discrepancies between what the system expects to have and what is really deployed in it.

## References

### [Clonezilla](https://clonezilla.org/)

- [Clonezilla Live - A small bootable GNU/Linux distribution for imaging and cloning](https://clonezilla.org/clonezilla-live.php)
  - [How to use Clonezilla live ?](https://clonezilla.org/clonezilla-usage/clonezilla-live-usage.php)
    - [This doc generally describes how to use Clonezilla live.](https://clonezilla.org/clonezilla-usage/general-live-use.php)

- [Clonezilla live doc (Updated: 3.1.1)](https://clonezilla.org/clonezilla-live-doc.php)
  - [Save disk image](https://clonezilla.org/fine-print-live-doc.php?path=clonezilla-live/doc/01_Save_disk_image)
  - [Restore disk image](https://clonezilla.org/fine-print-live-doc.php?path=clonezilla-live/doc/02_Restore_disk_image)
