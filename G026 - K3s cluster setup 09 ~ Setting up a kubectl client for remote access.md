...
2. To verify that you can connect to the cluster, try the `kubectl cluster-info` command from your client:
    ...
    E0911 12:10:25.655384   14465 memcache.go:265] "Unhandled Error" err="couldn't get current server API group list: Get \"https://10.4.1.1:6443/api?timeout=32s\": tls: failed to verify certificate: x509: certificate is valid for 0.0.0.0, 10.43.0.1, 127.0.0.1, 172.16.1.1, ::1, not 10.4.1.1"
    E0911 12:10:25.663803   14465 memcache.go:265] "Unhandled Error" err="couldn't get current server API group list: Get \"https://10.4.1.1:6443/api?timeout=32s\": tls: failed to verify certificate: x509: certificate is valid for 0.0.0.0, 10.43.0.1, 127.0.0.1, 172.16.1.1, ::1, not 10.4.1.1"
    E0911 12:10:25.671855   14465 memcache.go:265] "Unhandled Error" err="couldn't get current server API group list: Get \"https://10.4.1.1:6443/api?timeout=32s\": tls: failed to verify certificate: x509: certificate is valid for 0.0.0.0, 10.43.0.1, 127.0.0.1, 172.16.1.1, ::1, not 10.4.1.1"
    E0911 12:10:25.678646   14465 memcache.go:265] "Unhandled Error" err="couldn't get current server API group list: Get \"https://10.4.1.1:6443/api?timeout=32s\": tls: failed to verify certificate: x509: certificate is valid for 0.0.0.0, 10.43.0.1, 127.0.0.1, 172.16.1.1, ::1, not 10.4.1.1"
    E0911 12:10:25.686269   14465 memcache.go:265] "Unhandled Error" err="couldn't get current server API group list: Get \"https://10.4.1.1:6443/api?timeout=32s\": tls: failed to verify certificate: x509: certificate is valid for 0.0.0.0, 10.43.0.1, 127.0.0.1, 172.16.1.1, ::1, not 10.4.1.1"

    To further debug and diagnose cluster problems, use 'kubectl cluster-info dump'.
    Unable to connect to the server: tls: failed to verify certificate: x509: certificate is valid for 0.0.0.0, 10.43.0.1, 127.0.0.1, 172.16.1.1, ::1, not 10.4.1.1
    ~~~

    All those errors you see are a consecuence of a particular behaviour regarding the certificate autogenereted by the server node. The server node's external IP is not included in its own certificate, probably because it is considered that the public IP of a Kubernetes cluster should be associated to a proper non-autogenerated certificate. But, for a homelab setup like the one explained in this guide, the autogenerated one is good enough. Thankfully, there is an easy way to fix this issue.

3. Open a shell in your K3s server node and execute the following:

    ~~~sh
    $ sudo k3s server --tls-san "10.4.1.1"
    ~~~

4. Restart the K3s server service:
...
5. Copy again the contents of the `/etc/rancher/k3s/k3s.yaml` file from your K3s server node into your `kubectl` client system. Do not forget to change the server URL to make it point to your server node:
...
6. Try again the `kubectl cluster-info` command from your client system:
    ...
    This is the correct output to get from your K3s cluster, which means that now you can manage it remotely.
...
- [ServerFault. How to access k3s cluster created in an azure vm with public and private ip?](https://serverfault.com/a/1147283)
