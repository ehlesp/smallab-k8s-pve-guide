...
2. To verify that you can connect to the cluster, try the `kubectl cluster-info` command from your client:
    ...
    All those errors you see are a consecuence of a particular behaviour regarding the certificate autogenerated by the server node. The server node's external IP is not included in its own certificate, probably because it is considered that the public IP of a Kubernetes cluster should be associated to a proper non-autogenerated certificate. But, for a homelab setup like the one explained in this guide, the autogenerated one is good enough. The next steps solve this issue.
...
6. Try the `kubectl cluster-info` command again from your client system:
