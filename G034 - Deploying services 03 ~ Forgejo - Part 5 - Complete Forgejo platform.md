...
In this part you will finish the Forgejo platform deployment procedure. Here you will declare:
...
You will combine all these elements with the components' Kustomize subprojects you already created under a single main Kustomize project.
...
4. Open the `forgejo.k.output.yaml` file and compare it with the following one:
    ...
    Remember to check that the resources that get their names modified by Kustomize with an autogenerated suffix, `ConfigMaps` and `Secrets` in particular, get called by those changed names whereever they are used in this setup.
...
6. Just after applying the Kustomize project, monitor the deployment progress of your Forgejo platform:
    ...
    - The first components listed are the persitent volumes. Notice that not only the ones for the Forgejo platform appear, but also the ones for the Ghost platform. Remember that the `PersistentVolume` object is not namespaced in Kubernetes, so when you list them without a more concrete filter you will get all the ones enabled in your Kubernetes cluster. Also see how all the persistent volumes have the status `Bound`, indicating that they have been claimed.
...
1. When the pod for the Forgejo server is truly ready, browse into your Forgejo instance (found in `https://forgejo.homelab.cloud` for this guide) to get into the `Initial Configuration` form of Forgejo:
    ...
    > Given how this Forgejo instance has been configured, the server will keep on showing you this form until you properly finish its installation. It is possible to disable this step, but I have preferred to leave it enabled for creating an administrator user right in this form.
    >
...
4. The `Optional Settings` block is divided in three folded sections:
    ...
    Fill this form with a proper username, email address and password for your Forgejo administrator. For example, this guide sets the Forgejo administrator's username as `forgejoadm` and its email as `forgejoadm@forgejo.homelab.cloud` (this email does not exist, but it will suffice to fill the corresponding field), together with a long strong password.
...
5. The final line is a reminder to indicate that all this setup will be saved in Forgejo's `/var/lib/gitea/custom/conf/app.ini` configuration file:
...
7. A few seconds after pressing the `Install Forgejo` button, the Forgejo server will render a loading page with an animation of its logo while it finishes the installation:
...
8. After a minute, the installation process will end and will automatically redirect you into Forgejo's dashboard logged in as the administrator user you specified in the `Initial Configuration` form:
...
- [Reference. Kubernetes API. Config and Storage Resources](https://kubernetes.io/docs/reference/kubernetes-api/config-and-storage-resources/)
...
- [Rancher. GitHub. Local Path Provisioner](https://github.com/rancher/local-path-provisioner)

- [K3s. GitHub. Issue. Using "local-path" in persistent volume requires sudo to edit files on host node?](https://github.com/k3s-io/k3s/issues/1823)
...
- [Helm. GitHub. Issue. distinguish unset and empty values for storageClassName](https://github.com/helm/helm/issues/2600)
